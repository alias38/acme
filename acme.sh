...
green "例：原二级域名 x.xxxx.com 或 y.xxxx.com ，在cloudflare中重命名其中的x名称，确定并生效"
echo
...
green "root目录下的域名证书申请成功或已存在！域名证书（cert.crt）和密钥（private.key）已保存到 /root/acme文件夹" 
